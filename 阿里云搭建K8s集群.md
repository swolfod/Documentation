...
配置文件（[来源](https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.11.2/deploy/static/provider/cloud/deploy.yaml)，将镜像上传到阿里云并替换）：
```
# ingress-nginx.yaml
apiVersion: v1
kind: Namespace
  ...
  template:
    ...
    spec:
      ...
      - args:
        ...
        image: bucket-registry.cn-shanghai.cr.aliyuncs.com/k8s/ingress-nginx/controller:v1.11.2
      ...
      - args:
        ...
        - name: POD_NAMESPACE
          valueFrom:
            fieldRef:
              fieldPath: metadata.namespace
        image: bucket-registry.cn-shanghai.cr.aliyuncs.com/k8s/ingress-nginx/kube-webhook-certgen:v1.4.3
        imagePullPolicy: IfNotPresent
        name: create
        securityContext:
          allowPrivilegeEscalation: false
